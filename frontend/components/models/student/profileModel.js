...
import { asyncProfileUpdate } from "@/store/Actions/studentActions";
...
export default function profileModel() {
  ...
  const {student , errors ,  isAuthenticated} = useSelector(state => state.studentReducer)
  ...
  const formHandyy = (e) => {
    ...
    const formDataObj = {}
    formdata.set("profile", e.target.profile.files[0]);
    formdata.forEach((value, key) => (formDataObj[key] = value));
    console.log(formDataObj);
    dispatch(asyncProfileUpdate(formdata))
    setflag(!flag);
    toast.success("Student Avtar updated !!")
