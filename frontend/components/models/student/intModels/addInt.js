...
import { asyncAddInternShips, asyncAddJobs } from "@/store/Actions/studentActions";
...
const addInt = ()=> {
  ...
  const formHandyy = (data) => {
    dispatch(asyncAddInternShips(data))
          ...
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1">
                <h1 className="text-2xl font-semibold">Add Internships</h1>
              </ModalHeader>
              <ModalBody>
                {flag ? (
